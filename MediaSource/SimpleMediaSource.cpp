...
)
{
  ...
  RETURN_IF_FAILED(MakeAndInitialize<SimpleMediaStream>(&_videoStream, this, StreamType::Video));
  RETURN_IF_FAILED(MakeAndInitialize<SimpleMediaStream>(&_depthStream, this, StreamType::Depth));

  _streams[0] = _videoStream;
  _streams[1] = _depthStream;

  IMFStreamDescriptor* descriptors[] = {
    _videoStream.Get()->_spStreamDesc.Get(),
    _depthStream.Get()->_spStreamDesc.Get()
  };

  RETURN_IF_FAILED(MFCreatePresentationDescriptor(NUM_STREAMS, descriptors, &_spPresentationDescriptor));

  _wasStreamPreviouslySelected[0] = false;
  _wasStreamPreviouslySelected[1] = false;
...
)
{
  ...
  {
    _videoStream.Get()->Shutdown();
    _videoStream.Reset();
  }

  if (_depthStream != nullptr)
  {
    _depthStream.Get()->Shutdown();
    _depthStream.Reset();
  }
...
)
{
  ...
  for (auto i = 0u; i < count; i++)
  {
    ...
    RETURN_IF_FAILED(pPresentationDescriptor->GetStreamDescriptorByIndex(i,
      &selected,
      &streamDesc));
    ...
    if (selected)
    {
      ...
      MediaEventType met = (_wasStreamPreviouslySelected[streamIndex] ? MEUpdatedStream : MENewStream);

      auto& stream = _streams[streamIndex];
      // Update our internal PresentationDescriptor
      RETURN_IF_FAILED(_spPresentationDescriptor->SelectStream(i));
      RETURN_IF_FAILED(stream->SetStreamState(MF_STREAM_STATE_RUNNING));
      RETURN_IF_FAILED(stream.As(&spunkStream));
      ...
      // stream event queue.
      RETURN_IF_FAILED(stream.Get()->QueueEvent(MEStreamStarted,
        GUID_NULL,
        S_OK,
        &startTime));

      _wasStreamPreviouslySelected[streamIndex] = selected;
    }
  }
...
)
{
  ...
  for (auto i = 0u; i < count; i++)
  {
    BOOL selected = false;
    ComPtr<IMFStreamDescriptor> streamDesc;
    RETURN_IF_FAILED(_spPresentationDescriptor->GetStreamDescriptorByIndex(i,
      &selected,
      &streamDesc));
    ...
    if (selected)
    {
      ...
      RETURN_IF_FAILED(stream.Get()->GetStreamState(&state));
      _wasStreamPreviouslySelected[streamIndex] = (state == MF_STREAM_STATE_RUNNING);
      RETURN_IF_FAILED(stream.Get()->SetStreamState(MF_STREAM_STATE_STOPPED));
      _spPresentationDescriptor->DeselectStream(i);
      RETURN_IF_FAILED(stream.Get()->QueueEvent(MEStreamStopped, GUID_NULL, hr, &stopTime));
    }
  }
...
)
{
  ...
  *ppAttributes = nullptr;
  ...
  {
    auto& stream = _streams[dwStreamIdentifier];

    *ppAttributes = stream.Get()->_spAttributes.Get();
    (*ppAttributes)->AddRef();
  }
...
)
{
  ...
  if (_spEventQueue == nullptr || _videoStream == nullptr || _depthStream == nullptr)
  {
    return E_UNEXPECTED;
  }

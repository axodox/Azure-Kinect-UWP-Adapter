...
)
{
  ...
  case StreamType::Depth:
    ...
    _format = D3DFMT_D16;
    _spMediaType->SetGUID(MF_MT_MAJOR_TYPE, MFMediaType_Video);
    _spMediaType->SetGUID(MF_MT_SUBTYPE, MFVideoFormat_D16);
    break;
  }
  _bytesPerRow = _width * _bytesPerPixel;
  _bytesPerFrame = _height * _bytesPerRow;
  ...
  MFSetAttributeSize(_spMediaType.Get(), MF_MT_FRAME_SIZE, _width, _height);
  ...
  RETURN_IF_FAILED(MFCreateStreamDescriptor((DWORD)_type, 1, _spMediaType.GetAddressOf(), &_spStreamDesc));
...
)
{
  ...
  const int rows = len / abs(pitch);

  LONGLONG curSysTimeInS = MFGetSystemTime() / (MFTIME)10000000;
  int offset = curSysTimeInS % rows;

  for (int r = 0; r < rows; r++)
  {
    int grayColor = r + offset;
    memset(pBuf + (pitch * r), (BYTE)(grayColor % rows), pitch);
  }
...
)
{
  ...
  RETURN_IF_FAILED(MFCreate2DMediaBuffer(_width,
    _height,
    _format,
    false,
    &outputBuffer));
...
)
{
  ...
  RETURN_IF_FAILED(pAttributeStore->SetUINT32(MF_DEVICESTREAM_STREAM_ID, (unsigned)_type));
  ...
  case StreamType::Video:
    ...
    break;
  case StreamType::Depth:
    RETURN_IF_FAILED(pAttributeStore->SetUINT32(MF_DEVICESTREAM_ATTRIBUTE_FRAMESOURCE_TYPES, _MFFrameSourceTypes::MFFrameSourceTypes_Depth));
    break;
  }
